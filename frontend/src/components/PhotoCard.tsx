import { CheckSquare, Download, Lock, Square, Trash2 } from "lucide-react";
...
export const PhotoCard = ({
  ...
  const canOpen = !photo.encrypted || canModify;
  ...
  return (
    ...
    >
      <EncryptedImage
        ...
        variant="thumb"
        alt={fileName}
        onClick={canOpen ? () => onOpen(photo) : undefined}
        className={`h-full w-full object-cover transition duration-500 group-hover:scale-105 ${canOpen ? "cursor-zoom-in" : "cursor-default"}`}
      />
      {!canModify && photo.encrypted && (
        <div className="pointer-events-none absolute inset-0 grid place-items-center bg-slate-900/20">
          <span className="inline-flex items-center gap-1 rounded-full bg-black/50 px-2.5 py-1 text-[10px] font-semibold text-white backdrop-blur">
            <Lock size={12} />
            Locked
          </span>
        </div>
